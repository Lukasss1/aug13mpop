# Milk Pop T13.3.21 — Public Production Commissioning
...
The candidate is `r4.10.9-t13.3.21-public-launch`. It contains **106 ordered upgrade migrations**, **108 fresh-install SQL entries** and **17 Edge Functions**. T13.3.21 preserves the complete T13.3.20 application and database behaviour. It adds no migration, RPC, permission, role or customer workflow. This handoff release removes stale deployment pointers, corrects the environment template, and provides one strict public-release entrypoint so a small operator cannot accidentally deploy an older or development candidate.
...
Use `verify-only` only when the intended backend already has the exact T13.3.20 ledger and a valid commissioning receipt. Use `fresh`, `adopt` or `upgrade` whenever the backend is new, unverified or behind T13.3.20. T13.3.21 itself adds no database migration.
...
The supported operator entrypoint is `npm run public:release`. It runs the current handoff contract and then delegates to the existing cryptographic release seal; it does not create a second release system.
...
`PRODUCTION GO — READY TO OPEN MILK POP TO THE PUBLIC`
