# Milk Pop T13.3.15 — Production Commissioning
...
The current candidate is `r4.10.7-t13.3.15-runtime-resilience`. It contains 104 ordered upgrade migrations, 106 fresh-install SQL entries and 17 Edge Functions. T13.3.15 keeps the T13.3.14 deployment hardening and adds bounded browser network deadlines across sign-in, public forms, authenticated reads/writes, document/media flows, SEO synchronisation and the Web Till. Those deadlines remain active through response-body consumption, not only until headers arrive. It also closes the unhandled image-upload transport path, makes the inline image control keyboard-operable, bounds duplicate toast growth, corrects recovery/access wording and treats write-time transport failures as unconfirmed outcomes that require a reload before retry. It makes no database, permission, role, RPC or business-workflow change, so the backend commissioning sequence remains unchanged.
