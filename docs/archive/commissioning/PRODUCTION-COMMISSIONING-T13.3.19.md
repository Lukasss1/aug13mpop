# Milk Pop T13.3.19 — Production Commissioning
...
The candidate is `r4.10.8-t13.3.19-release-integrity`. It contains **105 ordered upgrade migrations**, **107 fresh-install SQL entries** and **17 Edge Functions**. T13.3.19 adds one append-only database migration and closes the release-integrity gaps around bounded request parsing, JWT-claim decoding, anonymous request control, atomic POS pairing, staff account lifecycle truth, immutable timesheets, staff-document reconciliation, provider-backed e-mail reservation and audit truth.
...
`verify-only` is not appropriate when moving from T13.3.18 to T13.3.19 because the new migration must be applied.

The workflow must:

1. apply or verify all **105** ordered upgrade migrations (**107** SQL entries on a fresh installation);
...
8. prove disable/enable uses claimed recovery intents and exposes partial/reconciliation results;
9. prove staff-document HTTP 400 does not remove metadata, successful deletion retains a tombstone, and unconfirmed upload rollback creates a cleanup job;
...
16. run owner-MFA media and staff-document upload/view/delete smoke tests;
