# Milk Pop T13.3.17 — Production Commissioning
...
The current candidate is `r4.10.7-t13.3.17-operator-recovery`. It contains 104 ordered upgrade migrations, 106 fresh-install SQL entries and 17 Edge Functions. T13.3.17 preserves the T13.3.16 provider deadlines and adds browser-level recovery around the remaining operator friction points: public cloud refreshes are single-flight and retry immediately when connectivity returns; owner document and CV actions reserve a secure destination tab before awaiting signed URLs so browser pop-up protection cannot silently block them; password recovery uses semantic native forms and bounded post-success navigation; staff document fields carry programmatic labels; and legacy-sale clipboard feedback is now confirmed rather than optimistic. It makes no database, permission, role, RPC, financial or business-workflow change, so the backend commissioning sequence remains unchanged.
