# Milk Pop T13.3.25 — Public Website Commissioning
...
This is the current commissioning authority for `r4.10.13-t13.3.25-local-preflight-trust-split`.
...
T13.3.25 retains the T13.3.23 route closure and T13.3.22 public-store projection and additionally proves that duplicate store names and long vacancy titles still produce unique, bounded, directly reloadable URLs. It also reconciles every current operator document to the 14-function public deployment.
...
The local preflight validates the reviewed source, production target, public trust anchor and configuration markers. It deliberately does **not** request the production private signing key; that key remains only in the protected GitHub `production` environment. The protected release workflow performs the mandatory private/public key match before sealing.
