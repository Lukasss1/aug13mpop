# Milk Pop T13.3.18 — Production Commissioning
...
The current candidate is `r4.10.7-t13.3.18-storage-resilience`. It contains 104 ordered upgrade migrations, 106 fresh-install SQL entries and 17 Edge Functions. T13.3.18 preserves every T13.3.17 operator-recovery guarantee and closes the final browser-storage failure path: a blocked `window.localStorage` getter can no longer prevent the Staff Portal module from loading; the Till lease degrades fail-closed with zero money capability; and malformed legacy held-sale data is surfaced as a recovery incident, preserved verbatim, protected from destructive overwrite and downloadable for manager-led review. It makes no database, permission, role, RPC, financial or business-workflow change, so the backend commissioning sequence remains unchanged.
...
1. apply or verify all **104** ordered upgrade migrations (**106** SQL entries on a fresh installation);
...
9. test Turnstile, exact-origin CORS and direct provider-backed email, including bounded provider-failure behaviour;
10. deliver a synthetic notification through the durable outbox, retain its provider message ID and verify retries reuse the same outbox idempotency key;
...
- With the network stalled or offline, public submissions leave their pending state within the bounded request window and show a retryable result.
...
- A stalled save, SEO recheck or image upload returns control to the operator instead of leaving the action pending indefinitely.
- The inline image-change control is reachable and operable by keyboard, and repeated identical failures do not flood the toast stack.
- The entity editor, Store Setup and VAT classification dialogs expose an accessible name, receive focus when opened and close with Escape only when no write is in flight.
- A simulated provider timeout leaves direct e-mail delivery visibly unconfirmed; the operator checks the e-mail log before retrying.
- With browser storage blocked, Staff Portal still loads and the Web Till remains read-only with an explicit storage warning.
- With malformed legacy held-sale storage, Till orders shows a recovery alert, preserves the raw value, offers a download and does not claim the queue is empty.
...
2. run official typecheck, lint ratchet, the request-timeout behavioural unit, full verification and Playwright/browser accessibility suites;
