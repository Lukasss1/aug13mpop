...
check('workflow installs and verifies PostgreSQL 17 for the seal lane', /Install and start PostgreSQL 17/.test(workflow) && /install-postgresql-17\.sh/.test(workflow) && /PostgreSQL 17 server required/.test(workflow) && !/service postgresql start/.test(workflow));
