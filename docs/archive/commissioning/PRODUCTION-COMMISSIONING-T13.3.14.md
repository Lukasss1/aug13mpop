# Milk Pop T13.3.14 — Production Commissioning
...
The current candidate is `r4.10.7-t13.3.14-deployment-polish`. It contains 104 ordered upgrade migrations, 106 fresh-install SQL entries and 17 Edge Functions. T13.3.14 adds a bounded deployment-polish layer: production-only Netlify build hardening, non-overlapping asset and release-marker cache policy, LCP resource discovery, image decode/layout metadata and production bundle budgets. It makes no database, permission, role or business-workflow change, so the commissioning sequence below remains unchanged.
