# Milk Pop T13.3.16 — Production Commissioning
...
The current candidate is `r4.10.7-t13.3.16-provider-resilience`. It contains 104 ordered upgrade migrations, 106 fresh-install SQL entries and 17 Edge Functions. T13.3.16 preserves the T13.3.15 browser-runtime hardening and adds end-to-end deadlines around the external Turnstile and Resend calls made by Edge Functions. Scheduled outbox retries now send one stable provider idempotency key per outbox row, while direct-send timeout or transport failures are recorded and reported as delivery **unconfirmed** so an operator checks the e-mail log before retrying. The three remaining legacy Admin overlays are now labelled modal dialogs with deterministic focus and Escape handling that respects in-flight write locks. It makes no database, permission, role, RPC, financial or business-workflow change, so the backend commissioning sequence remains unchanged.
